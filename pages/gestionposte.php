...
<form method="post" name="choixmachine">
	...
	<?php
		$req_machine = $db->query('SELECT t1.id_poste, t1.id_machine, machine
								FROM postes t1
	...
	if ($req_machine) {
		...
		while ($tbl_machine = mysqli_fetch_assoc($req_machine)) {
...
if (isset($_POST['id_poste']) AND $_POST['id_poste']!="-")	{
	...
	$tbl_poste = mysqli_fetch_assoc($req_poste);
?>
	<div id="machine">
			...
			<tr valign=top>
				<td>
					<table class="job2">			<!--job-->
						...
						<tr>
							...
							<td>
								<select name="id_tbljob" id="id_tbljob">
								<?php 
									$req_job = $db->query('
										SELECT tbljobs.id_tbljob, job, split, customer
										FROM tbljobs 
										...
										ORDER BY job, split
											') or die (mysql_error());							
									while($lst_job = mysqli_fetch_assoc($req_job))	{
										if ($tbl_poste['id_tbljob'] == $lst_job['id_tbljob'])
											echo '<option value="'.$lst_job['id_tbljob'].'" selected>'.$lst_job['customer'].'-'.$lst_job['job'].'-'.$lst_job['split'].'</option>';			
										else
											echo '<option value="'.$lst_job['id_tbljob'].'">'.$lst_job['customer'].'-'.$lst_job['job'].'-'.$lst_job['split'].'</option>';			
									}
								?>
								</select>
							</td>
						...
						<?php 
							...
							for($i=0; $i<count($titre); $i++){
								echo '
									<tr>
										<td>'.$titre[$i].'</td><td>';
								$req_enum = $db->query('SHOW COLUMNS FROM postes LIKE "'.$titresql[$i].'" ;') or die (mysql_error());							
				...
				<td>
					<table class="job2">			<!--	liste des differents equipements-->
						...
						<?php
						$req_enregistreur = $db->query('SELECT id_enregistreur, enregistreur FROM enregistreurs WHERE enregistreur_actif=1 ORDER BY enregistreur;') or die (mysql_error());
						if ($req_enregistreur) {
							echo '<option value="NULL">-</option>';
							while ($tbl_enregistreur = mysqli_fetch_assoc($req_enregistreur)) {
						...
						if ($req_extensometre) {
							echo '<option value="NULL">-</option>';
							while ($tbl_extensometre = mysqli_fetch_assoc($req_extensometre)) {
				...
				<td>
					<table class="job2">			<!--	liste des differents outillages-->
						...
						<?php
						$req_outillage = $db->query("SELECT id_outillage, outillage
							FROM outillages
						...
						if ($req_outillage) {
							echo '<option value="NULL">-</option>';
							while ($tbl_outillage = mysqli_fetch_assoc($req_outillage)) {
				...
				<td>
					<table class="job2">			<!--	liste des differents equipements de temperature-->
						...
						<?php
						$req_chauffage = $db->query('SELECT id_chauffage, chauffage FROM chauffages WHERE chauffage_actif=1 ORDER BY chauffage;') or die (mysql_error());
						if ($req_chauffage) {
							echo '<option value="NULL">-</option>';
							while ($tbl_chauffage = mysqli_fetch_assoc($req_chauffage)) {
						...
						<?php
						$req_ind_temp = $db->query('SELECT id_ind_temp, ind_temp FROM ind_temps WHERE ind_temp_actif=1 ORDER BY ind_temp;') or die (mysql_error());
						if ($req_ind_temp) {
							echo '<option value="NULL">-</option>';
							while ($tbl_ind_temp = mysqli_fetch_assoc($req_ind_temp)) {
	...
	<div id="historique">
		<table class="job2">
			...
			<?php 
				$req='SELECT customer, job, split, cartouche_stroke, cartouche_load, cartouche_strain, enregistreur, extensometre, o1.outillage as outillage_top, o2.outillage as outillage_bot, chauffage, i1.ind_temp as ind_temp_top, i2.ind_temp as ind_temp_strap, i3.ind_temp as ind_temp_bot,  IF( compresseur = 1,  "&#10004;",  "" ) as compresseur, date
				FROM postes 
				...
				LEFT JOIN info_jobs ON info_jobs.id_info_job=tbljobs.id_info_job
				WHERE id_machine='.$tbl_poste['id_machine'].' ORDER BY date DESC LIMIT 1,5';
//				echo $req;
				$req_historique = $db->query($req) or die (mysql_error());
				if ($req_historique) {
					while ($tbl_historique = mysqli_fetch_assoc($req_historique)) {
						...
						echo'<tr><td>'.$tbl_historique['date'].'</td><td>'.$tbl_historique['customer'].'-'.$tbl_historique['job'].'-'.$tbl_historique['split'];
